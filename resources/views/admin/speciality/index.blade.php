...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">{{ strtoupper($type) }} Specialities</h1>
    </div>
    <!-- /.col-lg-12 -->
</div>
        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    ...
                    List of {{ ($routeName == 'speciality.index.type') ? 'Active' : 'Trashed' }} categories
                </div>
                <!-- /.panel-heading -->
                        ...
                        <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                            ...
                            @forelse($categories as $category)
                                <tr class="odd gradeX">
                                    ...
                                    <td>{{ $category->name }}</td>
                                    <td class="center">
                                    @if($routeName == 'speciality.index.type')
                                       
                                        <a href="{{ route('speciality.edit' , ['speciality' => $category->id]) }}"><i class="fa fa-edit"></i></a> &nbsp;
                                        <a href="" onclick="deletecategory( {{ $category->id }} )"><i class="fa fa-trash"></i></a>
                                        <form action="{{ route('speciality.destroy' , ['speciality' => $category->id]) }}" method="POST" id="delete-form{{ $category->id }}" style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    @else
                                        <a href="javascript:void(0)" onclick="backToList( {{ $category->id }} )"><i class="fa fa-recycle"></i></a>
                                        <form action="{{ route('speciality.trash-back') }}" method="POST" id="trash-form{{ $category->id }}" style="display: none;">
                                            @csrf
