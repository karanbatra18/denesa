...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">{{ (Route::currentRouteName() == 'admin.post.create') ? 'ADD' : 'EDIT' }} POST</h1>
    </div>
    <!-- /.col-lg-12 -->
</div>
        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <div class="row">
                        <form role="form" action="{{ (Route::currentRouteName() == 'admin.post.create') ? route('admin.post.store') : route('admin.post.update', ['post' => $post->id ])  }}" method="post">
                            @csrf
                        <div class="col-lg-6">
                                ...
                                <div class="form-group">
                                    <label>Title</label>
                                    <input class="form-control" name="title" placeholder="Enter the title" value="{{ isset($post->title) ? $post->title : old('title') }}">
                                    @error('title')
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="short_description" class="form-control" id="myeditor1" rows="3">{{ (old('short_description') != null) ? old('short_description') : (isset($post) ? $post->short_description : null) }}</textarea>
                                ...
                                <div class="form-group">
                                    <label>Status</label>
                                    <?php $status = isset($post->status) ? $post->status : old('status') ;?>
                                ...
                                <div class="form-group">
                                    ...
                                    <input class="form-control" name="published_at" placeholder="Enter Publish Date" value="{{ isset($post->published_at) ? date('Y-m-d',strtotime($post->published_at)) : old('published_at') }}" readonly>
                                    @error('published_at')
                                    <label class="error">{{ $message }}</label>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="category_id[]" class="boot-multiselect-demo" multiple="multiple" style="display:none">
                                        @forelse($categories as $category)
                                            <option @if( $category->id == old('category_id') || (isset($categoryPosts) && in_array($category->id,$categoryPosts))) selected  @endif value="{{ $category->id }}">{{ $category->name }}</option>
                                        @empty
                                ...
                                <div class="form-group">
                                    ...
                                    <input id="hidden-image-feature_type"
                                           type="hidden"
                                           name="featured_image"
                                           value="{{ $post->featured_image ?? null }}">
                        ...
                        <div class="col-lg-6">
                            <div class="form-group">
                                <label>Seo URL</label>
                                <input class="form-control" name="slug" placeholder="Enter the Slug" value="{{ isset($post->slug) ? $post->slug : old('slug') }}">
                                @error('slug')
                                <label class="error">{{ $message }}</label>
                                @enderror
                            </div>
                            <div class="form-group">
                                ...
                                <input class="form-control" name="meta_title" placeholder="Enter the Seo Title" value="{{ isset($post->meta_title) ? $post->meta_title : old('meta_title') }}">
                                @error('meta_title')
                                <label class="error">{{ $message }}</label>
                                @enderror
                            </div>
                            <div class="form-group">
                                ...
                                <textarea name="meta_description" class="form-control" id="myeditor2" rows="3">{{ (old('meta_description') != null) ? old('meta_description') : (!empty($post) ? $post->meta_description : null) }}</textarea>
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="description" class="form-control" id="myeditor" rows="3">{{ (old('description') != null) ? old('description') : (!empty($post) ? $post->description : null) }}</textarea>
                                @error('description')
                                <label class="error">{{ $message }}</label>
                                @enderror
                            </div>
